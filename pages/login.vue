  ...
  <div class="auth-page">
    <div class="auth-card">
      <div class="auth-header">
        ...
        <h2 class="auth-title">{{ showOtpState ? 'Verifikasi Email Akun' : 'Masuk ke Merchant Portal' }}</h2>
        <p class="auth-sub">
          {{ showOtpState ? `Akun Anda belum aktif. Masukkan kode 6-digit OTP yang dikirim ke ${otpEmail}.` : 'Kelola Kunci API & lihat mutasi real-time' }}
        </p>
      </div>

      <form v-if="!showOtpState" novalidate @submit.prevent="handleLogin">
        <div class="fg">
          <label class="fl">Email</label>
          <input type="email" class="fi" v-model="form.email" placeholder="nama@toko.com" />
        </div>
      ...
      <form v-else novalidate @submit.prevent="handleVerifyOtp">
        ...
        <button type="submit" class="btn btn-primary btn-full mb-xs" :disabled="loading || otpCode.length < 6">
          {{ loading ? 'Memverifikasi...' : 'Verifikasi & Masuk Akun' }}
        </button>
      ...
      <div class="auth-foot">
        Belum punya akun? <NuxtLink to="/register" class="al">Daftar Sekarang</NuxtLink>
      </div>
    </div>
  </div>
</template>
...
async function handleLogin() {
  ...
  if (!isAllowedEmailDomain(form.value.email) && form.value.email.toLowerCase().trim() !== 'admin@warungerik.com') {
  ...
  if (!form.value.password) {
    swal.error('Password Wajib Diisi', 'Silakan masukkan password Anda.');
    return;
  }
  ...
  if (!turnstileToken) {
    swal.error('CAPTCHA Wajib Dicentang', 'Silakan centang verifikasi CAPTCHA terlebih dahulu sebelum masuk.');
    return;
  }

  loading.value = true;
  const r = await login(form.value.email, form.value.password, turnstileToken);
  loading.value = false;

  if (r.success) {
    if (r.role === 'ADMIN') navigateTo('/admin');
    else navigateTo('/dashboard');
  } else if (r.requireOtp) {
    showOtpState.value = true;
    otpUserId.value = r.userId;
    otpEmail.value = r.email;
    startTimer();
    swal.info('Kode OTP Terkirim', r.message || `Silakan periksa email ${r.email} untuk mendapatkan 6 digit kode OTP.`);
  } else {
    swal.error('Gagal Masuk', r.message);
    if (process.client && window.turnstile && turnstileContainer.value) {
...
.auth-card { background: #fff; border: 1px solid #E2E8F0; border-radius: 16px; padding: 32px 28px; width: 100%; max-width: 420px; box-shadow: 0 4px 20px rgba(0,0,0,0.06); }
